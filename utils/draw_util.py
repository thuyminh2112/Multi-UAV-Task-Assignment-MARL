...
def update(ax, env, uav_plots, target_plots, target_search_patches, frame, frames, num_steps, interval=2, paint_all=True):
    global text_obj
    ...
    for i in range(env.m_task):
        ...
        if target_x and target_y:  # Ensure the lists are not empty
            ...
            target_search_patches[i].center = (target_x[-1], target_y[-1])
        else:
            print(f"Warning: Target {i} position list is empty at frame {frame}.")

    text_str = (
        f"total completed w = {env.tot_completed_workload[frame]}\n"
    )
...
def draw_animation(config, env, num_steps, ep_num, frames=100):
    ...
    target_plots = [ax.scatter([], [], marker='o', color='r', linestyle='None', s=3,alpha=1) for _ in range(env.m_task)]
    target_search_patches = [patches.Circle((0, 0), 100, color='lightcoral', linestyle='None', alpha=0.2) for target in env.task_list]
    for patch in target_search_patches:
        ax.add_patch(patch)
    # ax.scatter(100, 100, marker='s', c='black', s=50)
    task_circle = patches.Circle((100, 100), 100, color='gray', alpha=0.15, linestyle='none', edgecolor='none')
    ...
    for frame in range(0, num_steps, step_interval):
        update(ax, env, uav_plots, target_plots, target_search_patches, frame, frames, num_steps)
        plt.draw()
...
def plot_reward_curve(config, return_list, name, x_label):
